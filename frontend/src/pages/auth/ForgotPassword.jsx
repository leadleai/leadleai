...
import AuthShell, { ConfigWarning } from "./AuthShell";
...
export default function ForgotPassword() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // would let anyone enumerate your users.
      setSent(true);
  ...
  return (
    <AuthShell
      ...
      subtitle={sent ? "We've sent a password reset link to your inbox." : "Enter your email and we'll send you a reset link."}
    >
      {sent ? (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          className="space-y-6" data-testid="reset-sent">
          ...
          <p className="text-center text-sm text-neutral-400">
            ...
            <button onClick={() => setSent(false)} className="text-neutral-200 hover:underline">try again</button>.
          </p>
        ...
        <form onSubmit={submit} className="space-y-4" data-testid="forgot-form" noValidate>
          ...
          <div className="space-y-1.5">
            <Label>Email</Label>
            <Input data-testid="forgot-email" type="email" value={email} autoComplete="email"
              onChange={(e) => { setEmail(e.target.value); setError(""); }}
              placeholder="you@company.com" className="rounded-xl h-11" />
            {error && <p className="text-xs text-red-500" data-testid="forgot-email-error">{error}</p>}
          </div>
          <Button data-testid="forgot-submit" type="submit" disabled={loading || !configured}
            className="w-full h-11 rounded-full bg-white text-black hover:shadow-lg">
            {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Sending…</> : "Send reset link"}
          </Button>
          <p className="text-center text-sm text-neutral-500 mt-4">
            <Link to="/login" className="text-neutral-300 font-medium hover:underline">← Back to login</Link>
          </p>
        </form>
      )}
    </AuthShell>
