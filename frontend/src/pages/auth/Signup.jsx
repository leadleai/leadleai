    ...
    <AuthShell title="Start your free trial" subtitle="14 days free. No credit card required.">
      <form onSubmit={submit} className="space-y-4" data-testid="signup-form" noValidate>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label>First name</Label><Input data-testid="signup-firstname" value={form.firstName} onChange={(e) => set("firstName", e.target.value)} placeholder="Alex" className={`rounded-xl h-11 ${errors.firstName ? "border-rose-400" : ""}`} />{errors.firstName && <p className="text-xs text-rose-500">{errors.firstName}</p>}</div>
          <div className="space-y-1.5"><Label>Last name</Label><Input data-testid="signup-lastname" value={form.lastName} onChange={(e) => set("lastName", e.target.value)} placeholder="Johnson" className={`rounded-xl h-11 ${errors.lastName ? "border-rose-400" : ""}`} />{errors.lastName && <p className="text-xs text-rose-500">{errors.lastName}</p>}</div>
        </div>
        <div className="space-y-1.5"><Label>Work email</Label><Input data-testid="signup-email" type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@company.com" className={`rounded-xl h-11 ${errors.email ? "border-rose-400" : ""}`} />{errors.email && <p className="text-xs text-rose-500" data-testid="signup-email-error">{errors.email}</p>}</div>
        <div className="space-y-1.5"><Label>Password</Label><Input data-testid="signup-password" type="password" value={form.password} onChange={(e) => set("password", e.target.value)} placeholder="Create a password (min 8 chars)" className={`rounded-xl h-11 ${errors.password ? "border-rose-400" : ""}`} />{errors.password && <p className="text-xs text-rose-500" data-testid="signup-password-error">{errors.password}</p>}</div>
        ...
        {errors.terms && <p className="text-xs text-rose-500 -mt-2">{errors.terms}</p>}
        <Button data-testid="signup-submit" type="submit" disabled={loading} className="w-full h-11 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:shadow-lg">
