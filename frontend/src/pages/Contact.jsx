        ...
        <div className="md:col-span-2 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6 sm:p-8">
          {sent ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8" data-testid="contact-success">
              ...
              <p className="text-neutral-500 mt-2">Thanks {form.name.split(" ")[0]}, we&apos;ll get back to you within 24 hours.</p>
            ...
            <form onSubmit={submit} className="space-y-4" data-testid="contact-form" noValidate>
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input data-testid="contact-name" value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Alex Johnson" className={`rounded-xl h-11 ${errors.name ? "border-neutral-400" : ""}`} />
                  {errors.name && <p className="text-xs text-neutral-500" data-testid="contact-name-error">{errors.name}</p>}
                </div>
                <div className="space-y-1.5">
                  <Label>Work email</Label>
                  <Input data-testid="contact-email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@company.com" className={`rounded-xl h-11 ${errors.email ? "border-neutral-400" : ""}`} />
                  {errors.email && <p className="text-xs text-neutral-500" data-testid="contact-email-error">{errors.email}</p>}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Textarea data-testid="contact-message" value={form.message} onChange={(e) => set("message", e.target.value)} placeholder="Tell us about your team and goals..." className={`rounded-xl min-h-28 ${errors.message ? "border-neutral-400" : ""}`} />
                {errors.message && <p className="text-xs text-neutral-500" data-testid="contact-message-error">{errors.message}</p>}
              </div>
              <Button data-testid="contact-submit" type="submit" disabled={loading} className="w-full h-11 rounded-full bg-white text-black hover:shadow-lg">
